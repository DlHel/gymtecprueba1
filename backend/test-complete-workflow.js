...
// Token de administrador (genera uno nuevo si es necesario)
...
async function testCompleteWorkflow() {
    ...
    try {
        ...
        const healthCheck = await axios.get(`${API_BASE}/test-db`, {
            headers: { 'Authorization': `Bearer ${AUTH_TOKEN}` }
        });
        console.log('✅ Backend disponible:', healthCheck.data.message);
        ...
        console.log('\n⏳ Esperando procesamiento automático de notificaciones...');
        ...
        console.log('\n⏳ Esperando nuevas notificaciones de actualización...');
        await new Promise(resolve => setTimeout(resolve, 2000));
        
        const finalNotifications = await axios.get(`${API_BASE}/notifications`, {
            headers: { 'Authorization': `Bearer ${AUTH_TOKEN}` }
        });
        ...
        try {
            ...
            console.log('⚠️  Estadísticas no disponibles:', statsError.response?.status || statsError.message);
        }
