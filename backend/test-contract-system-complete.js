...
async function testContractSystem() {
    ...
    try {
        ...
        const [clients] = await connection.execute('SELECT id, name, legal_name FROM Clients LIMIT 3');
        console.log(`✅ Clientes encontrados: ${clients.length}`);
        clients.forEach(client => {
            console.log(`   - ID: ${client.id}, Nombre: ${client.name}, Empresa: ${client.legal_name}`);
        });

        if (clients.length === 0) {
            console.log('⚠️ No hay clientes. Creando cliente de prueba...');
            const [result] = await connection.execute(`
                INSERT INTO Clients (name, email, phone, legal_name, address, contact_name, created_at)
                VALUES (?, ?, ?, ?, ?, ?, NOW())
            `, [
                'GymTech Solutions',
        ...
        const contractData = {
            ...
            contract_name: 'Contrato de Mantenimiento Premium',
            service_description: 'Contrato de mantenimiento completo para equipos de gimnasio',
        ...
        const insertSQL = `
            INSERT INTO Contracts (
                client_id, contract_number, contract_name, service_description, start_date, end_date, status,
        ...
        const [contractQuery] = await connection.execute(`
            SELECT c.*, cl.name as client_name, cl.legal_name as client_company, cl.email as client_email
            FROM Contracts c
