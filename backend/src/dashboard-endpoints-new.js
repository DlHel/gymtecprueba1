    ...
    const queries = [
        ...
        new Promise((resolve, reject) => {
            db.all(`
                ...
                FROM Users u
                LEFT JOIN Tickets t ON t.assigned_to = u.id AND t.status NOT IN ('Cerrado', 'Completado')
...
app.get('/api/dashboard/kpis-enhanced', authenticateToken, (req, res) => {
    ...
    const queries = [
        ...
        new Promise((resolve, reject) => {
            db.all(`SELECT COUNT(DISTINCT user_id) as total FROM Attendance WHERE DATE(check_in_time) = CURDATE()`, [], (err, rows) => {
                if (err) reject(err);
                else resolve({ metric: 'attendance_today', value: rows[0].total });
            });
        }),
